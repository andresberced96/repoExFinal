...
def obtenerDatosUsuario(request):
    ...
    try:
        ...
        dataUsuario = datosUsuario.objects.get(usrRel=idUsuario)
        return JsonResponse({
            'uname': objUsuario.username,
            'nUsuario': objUsuario.first_name,
            'apUsuario': objUsuario.last_name,
            'mailUsuario': objUsuario.email,
            'celular': dataUsuario.nroCelular,
            'profesion': dataUsuario.profesion,
            'perfil': dataUsuario.perfilUsuario
        })        
    except:
        return JsonResponse({
            'uname': 'SIN DATOS',
...
def actualizarUsuario(request):
    ...
    if request.method == 'POST':
        ...
        nombreUsuario = request.POST.get('nomUsuario')
        apellidoUsuario = request.POST.get('apeUsuario')
        profesionUsuario = request.POST.get('profeUsuario')
        nroCelular = request.POST.get('nCelular')
        perfilUsuario = request.POST.get('perUsuario')          

        usuarioUp = User.objects.get(id=idUsuario)
        usuarioUp.first_name = nombreUsuario
        usuarioUp.last_name = apellidoUsuario
        usuarioUp.save()

        dataUsuario = datosUsuario.objects.get(usrRel=idUsuario)
        dataUsuario.perfilUsuario = perfilUsuario
        dataUsuario.profesion = profesionUsuario
        dataUsuario.nroCelular = nroCelular
        dataUsuario.save()
